...
const createOrder = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  if ( !session ) {
    return res.status(401).json({ message: 'Debe estar autenticado para generar una orden' });
  }
  ...
  try {
    ...
    const subTotal = orderItems.reduce( (prev, current) => {
      ...
      if ( !currentPrice ) {
        throw new Error('Verificar carrito. Producto inexistente.');
      }
    ...
    if ( total !== backendTotal ) {
      throw new Error('Monto total no coincide')
    }
